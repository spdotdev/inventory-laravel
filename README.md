...
| `INVENTORY_CLIENT_ERRORS_RETENTION_DAYS` | `30` | Retention for `inventory:client-errors:prune` (scheduled daily by the package provider; `0` = keep forever). |
| `INVENTORY_DELETED_RETENTION_DAYS` | `30` | Retention for `inventory:deleted:prune`, which hard-deletes soft-deleted locations/shelves/products (and their orphaned images) past this window (scheduled daily by the package provider; `0` = keep forever). |
