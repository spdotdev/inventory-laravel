...
class LowStockApiTest extends TestCase
{
    ...
    public function test_count_reflects_low_stock_items_across_all_the_users_households(): void
    {
        ...
        $this->getJson("{$this->base}/low-stock/count")
            ->assertOk()
            ->assertJsonPath('data.count', 3);
    }

    public function test_count_excludes_null_threshold_above_threshold_and_missing_items(): void
    {
        $user = User::query()->create([
            'name' => 'Stan',
            'email' => 'stan@example.test',
        ...
        $household = Household::query()->create(['name' => 'Home', 'join_code' => 'DDDD4444']);
        $household->users()->attach($user);
        $shelf = $this->makeShelf($household);

        // Null threshold — must not count.
        Product::query()->create([
            'shelf_id' => $shelf->id,
            'name' => 'Snacks',
            'is_mandatory' => false,
        ...
        // Above threshold — must not count.
        Product::query()->create([
            'shelf_id' => $shelf->id,
            'name' => 'Bread',
            'is_mandatory' => false,
        ...
        // Mandatory and at zero — this is "missing", not "low" — must not count.
        Product::query()->create([
            'shelf_id' => $shelf->id,
            'name' => 'Eggs',
            'is_mandatory' => true,
        ...
        // Non-mandatory at zero with a threshold — this IS low stock — must count.
        Product::query()->create([
            'shelf_id' => $shelf->id,
            'name' => 'Juice',
            'is_mandatory' => false,
    ...
    {
        $this->getJson("{$this->base}/low-stock/count")->assertStatus(401);
    }
}
